...
let procesarTecladoDeAlien key state =
    if state.AlienEstado = Vivo then 
        match key with  
        | ConsoleKey.P -> // ⏸️ Si presiona P, activamos la pausa
...
let rec mainLoop state =
    ...
    | Paused ->
        ...
        mostrarMensaje 15 10 ConsoleColor.Yellow "  ===================  "
        mostrarMensaje 15 11 ConsoleColor.Yellow "   Pausa      "
        mostrarMensaje 15 12 ConsoleColor.Gray   "  [P] Reanudar  [S] Salir "
        mostrarMensaje 15 13 ConsoleColor.Yellow "  ===================  "

        
        let teclaPausa = Console.ReadKey(true)
        
        match teclaPausa.Key with
        | ConsoleKey.P -> 
    
            Console.Clear() 
            mainLoop { newState with ProgramState = Running }
        | ConsoleKey.S -> 
           
            Console.Clear()
            mainLoop { newState with ProgramState = Terminated }
        | _ -> 
